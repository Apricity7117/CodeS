...
const LIVE_PLAN_MESSAGES_STORAGE_KEY = 'codex-web-local.live-plan-messages.v1'
const NEW_THREAD_COLLABORATION_MODE_CONTEXT = '__new-thread__'
const EVENT_SYNC_DEBOUNCE_MS = 220
const BACKGROUND_THREAD_PAGINATION_DELAY_MS = 250
const AUTO_THREAD_HISTORY_LOAD_LIMIT = 1000
...
function readStoredRecord(value: unknown): Record<string, unknown> | null {
  return value !== null && typeof value === 'object' && !Array.isArray(value)
    ? (value as Record<string, unknown>)
    : null
}

function normalizeStoredPlanStatus(value: unknown): UiPlanStep['status'] {
  return value === 'completed' || value === 'inProgress' || value === 'pending' ? value : 'pending'
}

function normalizeStoredPlanPresentation(value: unknown): UiPlanPresentation | undefined {
  return value === 'inline' || value === 'dock' ? value : undefined
}

function normalizeStoredPlan(value: unknown): UiPlanData | undefined {
  const record = readStoredRecord(value)
  if (!record) return undefined
  const rawSteps = Array.isArray(record.steps) ? record.steps : []
  const steps = rawSteps
    .map((row) => readStoredRecord(row))
    .filter((row): row is Record<string, unknown> => Boolean(row))
    .map((row) => ({
      step: readStoredString(row.step).trim(),
      status: normalizeStoredPlanStatus(row.status),
    }))
    .filter((row) => row.step.length > 0)
  const explanation = readStoredString(record.explanation).trim()
  const presentation = normalizeStoredPlanPresentation(record.presentation)
  if (steps.length === 0 && !explanation && !presentation) return undefined
  return {
...
function normalizeStoredLivePlanMessage(value: unknown, fallbackPresentation?: UiPlanPresentation): UiMessage | null {
  const record = readStoredRecord(value)
  if (!record) return null
  const id = readStoredString(record.id).trim()
  const text = readStoredString(record.text)
  const messageType = readStoredString(record.messageType)
  if (!id || messageType !== 'plan.live') return null
  const plan = normalizeStoredPlan(record.plan)
  const nextPlan = plan
    ? {
        ...plan,
        presentation: normalizeStoredPlanPresentation(plan.presentation) ?? fallbackPresentation,
      }
    : fallbackPresentation
      ? { steps: [], isStreaming: true, presentation: fallbackPresentation }
      : undefined
  const turnId = readStoredString(record.turnId).trim()
  const sentAtIso = readStoredString(record.sentAtIso).trim()
  return {
    id,
    role: 'assistant',
    text,
...
function loadLivePlanMessagesMap(): Record<string, UiMessage[]> {
  if (typeof window === 'undefined') return {}
  try {
    const raw = window.localStorage.getItem(LIVE_PLAN_MESSAGES_STORAGE_KEY)
    if (!raw) return {}
    const parsed = JSON.parse(raw) as unknown
    const record = readStoredRecord(parsed)
    if (!record) return {}
    const next: Record<string, UiMessage[]> = {}
    const selectedModes = loadSelectedCollaborationModeMap()
    for (const [threadId, value] of Object.entries(record)) {
      if (!Array.isArray(value)) continue
      const fallbackPresentation = readSelectedCollaborationMode(selectedModes, threadId) === 'plan' ? 'inline' : 'dock'
      const messages = value
        .map((row) => normalizeStoredLivePlanMessage(row, fallbackPresentation))
        .filter((row): row is UiMessage => Boolean(row))
        .slice(-3)
      if (messages.length > 0) next[threadId] = messages
    }
    return next
  } catch {
    return {}
...
function saveLivePlanMessagesMap(state: Record<string, UiMessage[]>): void {
  if (typeof window === 'undefined') return
  const entries = Object.entries(state).filter(([, messages]) => messages.length > 0)
  if (entries.length === 0) {
    window.localStorage.removeItem(LIVE_PLAN_MESSAGES_STORAGE_KEY)
    return
  }
  window.localStorage.setItem(LIVE_PLAN_MESSAGES_STORAGE_KEY, JSON.stringify(Object.fromEntries(entries)))
}
...
function loadProjectOrder(): string[] {
  ...
  try {
    ...
    for (const item of parsed) {
      ...
      const normalizedItem = normalizePathForUi(item).trim()
...
function loadProjectDisplayNames(): Record<string, string> {
  ...
  try {
    ...
    for (const [projectName, displayName] of Object.entries(parsed as Record<string, unknown>)) {
      const normalizedProjectName = typeof projectName === 'string' ? normalizePathForUi(projectName).trim() : ''
...
function buildUniqueLeafProjectNameLookup(groups: UiProjectGroup[]): Map<string, string> {
  const projectNamesByLeaf = new Map<string, string[]>()
  for (const group of groups) {
    const leafName = toProjectName(group.projectName)
    if (!leafName) continue
    const rows = projectNamesByLeaf.get(leafName) ?? []
    rows.push(group.projectName)
    projectNamesByLeaf.set(leafName, rows)
  }

  const lookup = new Map<string, string>()
  for (const [leafName, projectNames] of projectNamesByLeaf.entries()) {
    const uniqueProjectNames = [...new Set(projectNames)]
    if (uniqueProjectNames.length === 1) {
      lookup.set(leafName, uniqueProjectNames[0])
    }
  }
  return lookup
}

function resolveProjectOrderName(orderName: string, incomingGroups: UiProjectGroup[], uniqueLeafLookup: Map<string, string>): string {
  if (incomingGroups.some((group) => group.projectName === orderName)) return orderName
  return uniqueLeafLookup.get(orderName) ?? orderName
}

function migrateProjectDisplayNamesForGroups(
  displayNames: Record<string, string>,
  groups: UiProjectGroup[],
): Record<string, string> {
  const uniqueLeafLookup = buildUniqueLeafProjectNameLookup(groups)
  let changed = false
  const next = { ...displayNames }

  for (const [name, displayName] of Object.entries(displayNames)) {
    if (groups.some((group) => group.projectName === name)) continue
    const resolvedName = uniqueLeafLookup.get(name)
    if (!resolvedName || next[resolvedName] !== undefined) continue
    next[resolvedName] = displayName
    changed = true
  }

  return changed ? next : displayNames
}

function mergeProjectOrder(previousOrder: string[], incomingGroups: UiProjectGroup[]): string[] {
  const nextOrder: string[] = []
  const uniqueLeafLookup = buildUniqueLeafProjectNameLookup(incomingGroups)

  for (const projectName of previousOrder) {
    const resolvedProjectName = resolveProjectOrderName(projectName, incomingGroups, uniqueLeafLookup)
    if (!nextOrder.includes(resolvedProjectName)) {
      nextOrder.push(resolvedProjectName)
    }
  }
...
function areSkillArraysEqual(
  first?: Array<{ name: string; path: string }>,
  second?: Array<{ name: string; path: string }>,
): boolean {
  const left = Array.isArray(first) ? first : []
  const right = Array.isArray(second) ? second : []
  if (left.length !== right.length) return false
...
function areUiFileAttachmentsEqual(
...
function fileAttachmentsWithLocalImages(
  fileAttachments: UiMessage['fileAttachments'],
  imageUrls: UiMessage['images'],
): UiMessage['fileAttachments'] {
  const next = Array.isArray(fileAttachments) ? [...fileAttachments] : []
  for (const imageUrl of Array.isArray(imageUrls) ? imageUrls : []) {
    const path = extractLocalImagePathFromUrl(imageUrl.trim())
    if (!path) continue
    if (next.some((attachment) => attachment.path === path)) continue
...
function areMessageFieldsEqual(first: UiMessage, second: UiMessage): boolean {
  return (
    ...
    areSkillArraysEqual(first.skills, second.skills) &&
    areUiFileAttachmentsEqual(first.fileAttachments, second.fileAttachments) &&
    ...
    first.sentAtIso === second.sentAtIso
  )
}
...
function optimisticUserMessageMatchesPersisted(optimistic: UiMessage, persisted: UiMessage): boolean {
  return optimistic.role === 'user' &&
    persisted.role === 'user' &&
    optimistic.messageType === OPTIMISTIC_USER_MESSAGE_TYPE &&
    optimistic.text === persisted.text &&
    areStringArraysEqual(optimistic.images, persisted.images) &&
    areSkillArraysEqual(optimistic.skills, persisted.skills) &&
    (
      areUiFileAttachmentsEqual(optimistic.fileAttachments, persisted.fileAttachments) ||
      areUiFileAttachmentsEqual(
        fileAttachmentsWithLocalImages(optimistic.fileAttachments, optimistic.images),
        persisted.fileAttachments,
      )
    )
}

function readOptimisticUserMessageAnchorId(message: UiMessage): string {
  const rawPayload = message.rawPayload ?? ''
  return rawPayload.startsWith(OPTIMISTIC_USER_MESSAGE_META_PREFIX)
    ? rawPayload.slice(OPTIMISTIC_USER_MESSAGE_META_PREFIX.length)
    : ''
}

function findPersistedOptimisticUserMessageMatchIndex(
  optimistic: UiMessage,
  persisted: UiMessage[],
  consumedIndexes: Set<number>,
): number {
  const anchorId = readOptimisticUserMessageAnchorId(optimistic)
  const anchorIndex = anchorId
    ? persisted.findIndex((message) => message.id === anchorId)
    : -1
  if (anchorId && anchorIndex < 0) return -1

  for (let index = anchorIndex + 1; index < persisted.length; index += 1) {
    if (consumedIndexes.has(index)) continue
    if (optimisticUserMessageMatchesPersisted(optimistic, persisted[index])) {
      return index
    }
  }
  return -1
...
function removePersistedOptimisticUserMessages(optimistic: UiMessage[], persisted: UiMessage[]): UiMessage[] {
  if (optimistic.length === 0 || persisted.length === 0) return optimistic

  const consumedIndexes = new Set<number>()
  const next: UiMessage[] = []
  let changed = false
  for (const optimisticMessage of optimistic) {
    const matchIndex = findPersistedOptimisticUserMessageMatchIndex(
      optimisticMessage,
      persisted,
      consumedIndexes,
    )
...
function getWorkspaceProjectOrderNames(
  ...
  _duplicateLeafNames: Set<string>,
): string[] {
  ...
  return getWorkspaceProjectOrderPaths(rootsState).map((rootPath) => {
    ...
    return toHistoryProjectName(rootPath)
  })
}
...
function collectDuplicateProjectLeafNames(groups: UiProjectGroup[], rootsState: WorkspaceRootsState | null): Set<string> {
  ...
  for (const group of groups) {
    for (const thread of group.threads) {
      ...
      addPath(normalizedCwd || group.projectName)
    }
  }
...
function disambiguateProjectGroupsByCwd(
  ...
  for (const group of groups) {
    for (const thread of group.threads) {
      ...
      const projectName = normalizedCwd && duplicateLeafNames.has(leafName)
        ? toHistoryProjectName(normalizedCwd)
        : toHistoryProjectName(normalizedCwd || group.projectName)
...
export function filterGroupsByWorkspaceRoots(
  ...
  if (!rootsState || (rootsState.order.length === 0 && rootsState.projectOrder.length === 0)) return disambiguatedGroups
  return orderGroupsByWorkspaceProjectOrder(disambiguatedGroups, rootsState, duplicateLeafNames)
}
...
export function useDesktopState() {
  ...
  const livePlanMessagesByThreadId = ref<Record<string, UiMessage[]>>(loadLivePlanMessagesMap())
  ...
  const hasMoreThreadHistory = ref(false)
  const isLoadingMoreThreadHistory = ref(false)
  ...
  const messages = computed<UiMessage[]>(() => {
    ...
    const persisted = persistedMessagesByThreadId.value[threadId] ?? []
    const optimisticUser = removePersistedOptimisticUserMessages(
  ...
  async function retryPendingTurnWithFallback(threadId: string): Promise<void> {
    ...
    let retryOptimisticUserMessage: UiMessage | null = null

    try {
      await applyFallbackModelSelection(threadId)
      // 回放到兜底模型前先移除失败用户轮次，避免真实消息重复。
      let removedFailedUserTurn = false
      try {
      ...
      if (removedFailedUserTurn) {
        retryOptimisticUserMessage = appendOptimisticUserMessage(
          threadId,
          pending.text,
          pending.imageUrls,
          pending.skills,
      ...
      if (retryOptimisticUserMessage) {
        removeOptimisticUserMessage(threadId, retryOptimisticUserMessage.id)
      }
    } finally {
  ...
  function insertOptimisticThread(threadId: string, cwd: string, firstMessageText: string): void {
    ...
    const projectName = toHistoryProjectName(normalizedCwd)
  ...
  function pruneThreadScopedState(flatThreads: UiThread[]): void {
    ...
    const nextLivePlans = pruneThreadStateMap(livePlanMessagesByThreadId.value, activeThreadIds)
    if (nextLivePlans !== livePlanMessagesByThreadId.value) {
      livePlanMessagesByThreadId.value = nextLivePlans
      saveLivePlanMessagesMap(livePlanMessagesByThreadId.value)
    }
  ...
  function setPersistedMessagesForThread(threadId: string, nextMessages: UiMessage[]): void {
    ...
    const optimisticUserMessages = optimisticUserMessagesByThreadId.value[threadId] ?? []
    const nextOptimisticUserMessages = removePersistedOptimisticUserMessages(optimisticUserMessages, nextMessages)
    if (nextOptimisticUserMessages !== optimisticUserMessages) {
      if (nextOptimisticUserMessages.length === 0) {
        ...
        optimisticUserMessagesByThreadId.value = {
          ...optimisticUserMessagesByThreadId.value,
          [threadId]: nextOptimisticUserMessages,
        }
      }
    }
  ...
  function appendOptimisticUserMessage(
    ...
    const persisted = persistedMessagesByThreadId.value[threadId] ?? []
    const anchorId = persisted.at(-1)?.id ?? ''
    const nextMessage: UiMessage = {
      ...
      fileAttachments: fileAttachments.length > 0
        ? fileAttachments.map((file) => ({ label: file.label, path: file.path }))
        : undefined,
      messageType: OPTIMISTIC_USER_MESSAGE_TYPE,
      rawPayload: `${OPTIMISTIC_USER_MESSAGE_META_PREFIX}${anchorId}`,
      sentAtIso: new Date().toISOString(),
    }
    const previous = optimisticUserMessagesByThreadId.value[threadId] ?? []
    optimisticUserMessagesByThreadId.value = {
      ...optimisticUserMessagesByThreadId.value,
      [threadId]: [...previous, nextMessage],
    }
    return nextMessage
  }
  ...
  function normalizePlanPresentation(value: unknown): UiPlanPresentation | null {
    return value === 'inline' || value === 'dock' ? value : null
  }

  function resolveLivePlanPresentation(threadId: string, existingMessage?: UiMessage): UiPlanPresentation {
    const existingPresentation = normalizePlanPresentation(existingMessage?.plan?.presentation)
    if (existingPresentation) return existingPresentation

    const pendingMode = pendingTurnRequestByThreadId.value[threadId]?.collaborationMode
    if (pendingMode === 'plan') return 'inline'
    if (pendingMode === 'default') return 'dock'

    const selectedMode = readSelectedCollaborationMode(selectedCollaborationModeByContext.value, threadId)
    return selectedMode === 'plan' ? 'inline' : 'dock'
  }
  ...
  function readPlanUpdate(notification: RpcNotification): { threadId: string; message: UiMessage } | null {
    ...
    const plan: UiPlanData = {
      ...
      steps,
      isStreaming: true,
      presentation: resolveLivePlanPresentation(threadId),
    }
  ...
  function readPlanDelta(notification: RpcNotification): { threadId: string; message: UiMessage } | null {
    ...
    const nextPlan: UiPlanData | undefined = existing?.plan
      ? { ...existing.plan, isStreaming: true, presentation: resolveLivePlanPresentation(threadId, existing) }
      : { steps: [], isStreaming: true, presentation: resolveLivePlanPresentation(threadId) }
  ...
  function applyRealtimeUpdates(notification: RpcNotification): void {
    ...
    if (startedTurn) {
      ...
      setTurnActivityForThread(startedTurn.threadId, {
        ...
        details: turnActivityByThreadId.value[startedTurn.threadId]?.details ?? [],
  ...
  async function hydrateWorkspaceRootsStateIfNeeded(
    ...
    try {
      ...
      for (const rootPath of getWorkspaceProjectOrderPaths(rootsState)) {
        const projectName = toHistoryProjectName(rootPath)
      ...
      if (Object.keys(rootsState.labels).length > 0 || (rootsState.remoteProjects ?? []).length > 0) {
        ...
        for (const [rootPath, label] of Object.entries(rootsState.labels)) {
          const normalizedRootPath = normalizePathForUi(rootPath).trim()
          const projectNames = [toHistoryProjectName(rootPath)]
        ...
        for (const rootPath of rootsState.order) {
          ...
          if (nextLabels[projectName] !== undefined || nextLabels[projectName] === displayName) continue
          nextLabels[projectName] = displayName
          changed = true
        }
  ...
  function applyThreadGroups(groups: UiProjectGroup[], rootsState: WorkspaceRootsState | null): void {
    ...
    const migratedDisplayNames = migrateProjectDisplayNamesForGroups(projectDisplayNameById.value, visibleGroups)
    if (migratedDisplayNames !== projectDisplayNameById.value) {
      projectDisplayNameById.value = migratedDisplayNames
  ...
  function mergeThreadGroupPages(previous: UiProjectGroup[], incoming: UiProjectGroup[]): UiProjectGroup[] {
    ...
    for (const thread of threadById.values()) {
      ...
      const nextThread = thread.projectName === projectName ? thread : { ...thread, projectName }
      const existing = groupsByProject.get(projectName)
      if (existing) existing.push(nextThread)
      else groupsByProject.set(projectName, [nextThread])
    }
  ...
  function insertThreadSummaries(threads: UiThread[]): void {
    const normalizedThreads = threads
      .map((thread) => {
        const projectName = toHistoryProjectName(thread.cwd || thread.projectName)
        return thread.projectName === projectName ? thread : { ...thread, projectName }
      })
      .filter((thread, index, rows) => {
        return thread.id.trim().length > 0 && rows.findIndex((candidate) => candidate.id === thread.id) === index
      })
    if (normalizedThreads.length === 0) return

    const incomingGroups = normalizedThreads.map((thread) => ({
      projectName: thread.projectName,
      threads: [thread],
    }))
    loadedThreadListGroups = mergeThreadGroupPages(loadedThreadListGroups, incomingGroups)
  ...
  function loadedThreadListThreadCount(): number {
    return flattenThreads(loadedThreadListGroups).length
  }

  function canAutoLoadMoreThreadPages(): boolean {
    return loadedThreadListThreadCount() < AUTO_THREAD_HISTORY_LOAD_LIMIT
  }
  ...
  function scheduleRemainingThreadPages(rootsState: WorkspaceRootsState | null = loadedThreadListRootsState): void {
    ...
    if (!canAutoLoadMoreThreadPages()) return

    loadedThreadListRootsState = rootsState

    if (typeof window === 'undefined') {
      void loadRemainingThreadPages(rootsState, { manual: false })
      return
    }
    ...
    threadListBackgroundTimer = window.setTimeout(() => {
      ...
      if (!canAutoLoadMoreThreadPages()) return
      void loadRemainingThreadPages(loadedThreadListRootsState, { manual: false })
  ...
  async function loadRemainingThreadPages(
    ...
    if (!manual && !canAutoLoadMoreThreadPages()) return
    isLoadingRemainingThreadPages = true
    if (manual) {
      isLoadingMoreThreadHistory.value = true
    }
    ...
    try {
      ...
      hasMoreThreadHistory.value = page.nextCursor !== null
      ...
      if (manual) {
        isLoadingMoreThreadHistory.value = false
      }
      if (threadListNextCursor && !hasActiveInProgressThreads() && canAutoLoadMoreThreadPages()) {
  ...
  async function loadMoreThreadHistory(): Promise<void> {
    ...
    await loadRemainingThreadPages(loadedThreadListRootsState, { manual: true })
  }
  ...
  async function loadThreads() {
    ...
    try {
      ...
      threadListNextCursor = hasLoadedThreads.value && !hasLoadedAllThreadPages
        ...
        : page.nextCursor
      hasMoreThreadHistory.value = threadListNextCursor !== null
      hasLoadedAllThreadPages = page.nextCursor === null
      isThreadListFullyLoaded.value = hasLoadedAllThreadPages
      ...
      if (!hasLoadedAllThreadPages && canAutoLoadMoreThreadPages()) {
        scheduleRemainingThreadPages(rootsState)
      }
  ...
  async function refreshAncillaryState(): Promise<void> {
  ...
  function scheduleAncillaryStateRefresh(): void {
    const run = () => {
      void refreshAncillaryState()
    }
  ...
  async function startTurnForThread(
    ...
    const optimisticUserMessage = appendOptimisticUserMessage(
      threadId,
      normalizedText,
      normalizedImageUrls,
      normalizedSkills,
    ...
    try {
      ...
      removeOptimisticUserMessage(threadId, optimisticUserMessage.id)
      throw unknownError
    }
  }
