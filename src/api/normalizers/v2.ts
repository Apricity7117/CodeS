...
function readUuidV7Timestamp(value: unknown): string | undefined {
  if (typeof value !== 'string') return undefined
  const match = /^([0-9a-f]{8})-([0-9a-f]{4})-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu.exec(value.trim())
  if (!match) return undefined
  const timestamp = Number.parseInt(`${match[1]}${match[2]}`, 16)
  const earliestReasonableTimestamp = Date.UTC(2020, 0, 1)
  const latestReasonableTimestamp = Date.now() + 365 * 24 * 60 * 60 * 1000
  if (!Number.isFinite(timestamp) || timestamp < earliestReasonableTimestamp || timestamp > latestReasonableTimestamp) {
    return undefined
  }
  return new Date(timestamp).toISOString()
}

function readTimestampFromRecord(record: Record<string, unknown> | null | undefined): string | undefined {
  if (!record) return undefined
  return toIsoTimestamp(record.createdAt)
    ?? toIsoTimestamp(record.created_at)
    ?? toIsoTimestamp(record.timestamp)
    ?? toIsoTimestamp(record.time)
    ?? toIsoTimestamp(record.startedAt)
    ?? toIsoTimestamp(record.started_at)
    ?? toIsoTimestamp(record.completedAt)
    ?? toIsoTimestamp(record.completed_at)
    ?? toIsoTimestamp(record.updatedAt)
    ?? toIsoTimestamp(record.updated_at)
    ?? readUuidV7Timestamp(record.id)
}

function readMessageSentAtIso(item: ThreadItem, turn: Turn | undefined): string | undefined {
  return readTimestampFromRecord(item as unknown as Record<string, unknown>)
...
function readTurnDurationMs(turn: Turn): number {
  const rawTurn = turn as unknown as Record<string, unknown>
  const durationMs = readFiniteNumber(rawTurn.durationMs)
  if (durationMs !== null) return Math.max(0, durationMs)

  const startedAtMs = readTimestampMs(rawTurn.startedAt)
  const completedAtMs = readTimestampMs(rawTurn.completedAt)
...
function isHistoricalFinalAssistantCandidate(message: UiMessage): boolean {
  return message.role === 'assistant'
    && message.messageType !== WORKED_MESSAGE_TYPE
    && message.messageType !== 'agentMessage.live'
    && message.text.trim().length > 0
}

function findHistoricalFinalAssistantIndex(messages: UiMessage[]): number {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    if (isHistoricalFinalAssistantCandidate(messages[index])) return index
  }
  return -1
}

function isHistoricalWorkedProcessMessage(message: UiMessage): boolean {
...
function buildHistoricalWorkedMessage(turn: Turn, turnIndex: number): UiMessage {
  const turnId = typeof turn.id === 'string' && turn.id.trim().length > 0 ? turn.id.trim() : `turn-${turnIndex}`
  return {
    id: `turn-summary:${turnId}`,
    role: 'system',
    text: `Worked for ${formatTurnDuration(readTurnDurationMs(turn))}`,
...
function insertHistoricalWorkedMessage(turn: Turn, turnIndex: number, messages: UiMessage[]): UiMessage[] {
  if (turn.status !== 'completed') return messages
  if (messages.some((message) => message.messageType === WORKED_MESSAGE_TYPE)) return messages

  const finalAssistantIndex = findHistoricalFinalAssistantIndex(messages)
  if (finalAssistantIndex < 0) return messages

  const hasProcessMessage = messages.some((message, index) => index !== finalAssistantIndex && isHistoricalWorkedProcessMessage(message))
  if (!hasProcessMessage) return messages

  const next = [...messages]
  next.splice(finalAssistantIndex, 0, buildHistoricalWorkedMessage(turn, turnIndex))
  return next
}
...
export function normalizeThreadMessagesV2(payload: ThreadReadResponse, baseTurnIndex = 0): UiMessage[] {
  ...
  for (let turnOffset = 0; turnOffset < turns.length; turnOffset++) {
    ...
    for (const item of items) {
      const sentAtIso = readMessageSentAtIso(item, turn)
      for (const msg of toUiMessages(item)) {
        turnMessages.push({ ...msg, ...(sentAtIso ? { sentAtIso } : {}), turnId, turnIndex })
      }
    }
    messages.push(...insertHistoricalWorkedMessage(turn, turnIndex, turnMessages))
  }
  return messages
}
