<template>
  <div ref="rootRef" class="header-git-dropdown" :class="{ 'is-review-open': reviewOpen, 'is-inspector-menu': compactMenu }">
    <button
      ...
      @click="toggleOpen"
    >
      <IconCodexWorktree v-if="compactMenu" class="header-git-trigger-icon" />
      <IconCodexBranch v-else class="header-git-trigger-icon" />
      ...
      <IconCodexChevron v-if="!compactMenu" class="header-git-trigger-chevron" />
    </button>

    <div v-if="isOpen" ref="menuWrapRef" class="header-git-menu-wrap" :style="menuWrapStyle">
      <div class="header-git-menu">
        <button v-if="showReview" class="header-git-review-row" type="button" @click="emit('toggleReview')">
          <IconCodexEdit class="header-git-row-icon" />
          <span>{{ reviewOpen ? t('Review (Open)') : t('Review') }}</span>
        </button>

        <div v-if="showStateSummary" class="header-git-state">
        ...
        <div v-if="compactMenu" class="header-git-branch-section-label">{{ t('Branch') }}</div>
        ...
        <ul class="header-git-branches" role="listbox">
          <li v-for="branch in filteredBranches" :key="branch.value" class="header-git-branch-item">
            <div class="header-git-branch-row">
              ...
              >
                <IconCodexChevronRight class="header-git-expand-icon" :class="{ 'is-expanded': expandedBranch === branch.value }" />
              </button>
              <button
...
import { IconCodexBranch, IconCodexChevron, IconCodexChevronRight, IconCodexEdit, IconCodexWorktree } from '../icons/codex'
...
const compactMenu = computed(() => props.placement === 'inspector')
const showReview = computed(() => props.showReview !== false && !compactMenu.value)
const showStateSummary = computed(() => !compactMenu.value)
...
const statusMessage = computed(() => {
  ...
  if (compactMenu.value) return ''
  return props.dirty ? 'Tracked changes must be committed, stashed, or discarded before switching or resetting. Untracked files are allowed unless Git would overwrite them.' : ''
})
...
function updateMenuPosition(): void {
  if (!isOpen.value || !compactMenu.value || typeof window === 'undefined') {
    menuWrapStyle.value = {}
    return
  }

  const trigger = rootRef.value?.querySelector('.header-git-trigger')
  if (!(trigger instanceof HTMLElement)) return

  const rect = trigger.getBoundingClientRect()
  const viewportWidth = window.visualViewport?.width ?? window.innerWidth
  const viewportHeight = window.visualViewport?.height ?? window.innerHeight
  const menuWidth = Math.min(320, Math.max(240, viewportWidth - 24))
  const left = Math.min(
    Math.max(12, rect.left - menuWidth + 24),
    Math.max(12, viewportWidth - menuWidth - 12),
  )
  const top = Math.min(
    Math.max(12, rect.top - 28),
    Math.max(12, viewportHeight - 120),
  )
...
.header-git-dropdown.is-inspector-menu .header-git-menu-wrap {
  @apply z-[80];
}

.header-git-dropdown.is-inspector-menu .header-git-menu {
  @apply p-2 shadow-2xl shadow-zinc-900/15;
}
...
.header-git-branch-section-label {
  @apply px-2 pb-1 pt-0.5 text-xs font-medium leading-5 text-zinc-500;
}
