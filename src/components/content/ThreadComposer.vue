    ...
    >
      ...
      <div v-if="folderUploadGroups.length > 0" class="thread-composer-folder-chips">
        <span v-for="group in folderUploadGroups" :key="group.id" class="thread-composer-folder-chip">
          <IconCodexFolder class="thread-composer-folder-chip-icon" />
      ...
      <div v-if="standaloneFileAttachments.length > 0" class="thread-composer-file-chips">
        <span v-for="att in standaloneFileAttachments" :key="att.fsPath" class="thread-composer-file-chip">
          <IconCodexEdit class="thread-composer-file-chip-icon" />
      ...
      >
        ...
        <div v-if="isFileMentionOpen" class="thread-composer-file-mentions">
          <template v-if="fileMentionSuggestions.length > 0">
            ...
            >
              ...
              <IconCodexEdit v-else class="thread-composer-file-mention-icon-file" />
        ...
        >
          <IconCodexExpand2 v-if="isComposerExpanded" class="thread-composer-expand-icon" />
          <IconCodexExpand v-else class="thread-composer-expand-icon" />
        </button>
      </div>
      ...
      >
        ...
        >
          ...
          >
            ...
            <IconCodexStop v-else class="thread-composer-stop-icon" />
          </button>
          <button
            v-else
          ...
          >
            <IconCodexArrowUp class="thread-composer-submit-icon" />
          </button>
        </div>
      </div>
...
const shouldShowStopButton = computed(() => props.isTurnInProgress === true && !hasSubmitContent.value)
const canInterruptTurn = computed(() =>
  shouldShowStopButton.value &&
  !props.disabled &&
  Boolean(props.activeThreadId) &&
  props.isInterruptingTurn !== true &&
  props.isStopPending !== true,
)
...
function onInterrupt(): void {
  if (!canInterruptTurn.value) return
  emit('interrupt')
}
...
function onInputKeydown(event: KeyboardEvent): void {
  ...
  if (event.key === 'Escape' && canInterruptTurn.value) {
    event.preventDefault()
...
.thread-composer:has(.thread-composer-input-wrap--expanded) {
  @apply fixed inset-0 z-50 max-w-none p-3 sm:p-6;
  background-color: color-mix(in srgb, var(--codex-bg) 95%, transparent);
}
...
.thread-composer-expand {
  @apply absolute right-0.5 top-0.5 inline-flex h-8 w-8 items-center justify-center rounded-full border-0 shadow-sm transition disabled:cursor-not-allowed disabled:opacity-50;
  background-color: var(--codex-control-bg);
  color: var(--codex-muted-text);
}
.thread-composer-expand:hover:not(:disabled) {
...
.thread-composer-actions {
  @apply ml-auto flex min-w-0 items-center gap-1.5;
}
...
.thread-composer-submit:hover:not(:disabled) {
  opacity: 0.9;
}

.thread-composer-submit.thread-composer-submit--queue {
  background-color: var(--color-amber-600);
  color: white;
}
.thread-composer-submit.thread-composer-submit--queue:hover:not(:disabled) {
...
.thread-composer-stop:hover:not(:disabled) {
  opacity: 0.9;
}
