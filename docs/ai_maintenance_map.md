...
- 应用级纯配置、偏好读写、导出文本、账号展示文本、Skills Hub try 文案：先看 `src/app/`，再看 `src/App.vue` 的薄包装调用。
...
- Codex app-server bridge、inline media、session recovery：先看 `src/server/codexAppServerBridge.ts`。
...
- 本地 Skills 页面：先看 `src/components/content/SkillsHub.vue`、`src/server/skillsRoutes.ts` 和 `src/app/directoryTry.ts`。

## 目录职责

- `src/app/`：承接顶层应用的纯逻辑和稳定配置，例如 localStorage 偏好、设置帮助文案、账号显示文本、会话 Markdown 导出和 Skills Hub try 文案。这里不放 Vue 响应式状态。
...
- `src/server/`：Codex app-server bridge、本地资源服务、Skills routes 等 Node 侧能力。
